...
class Products
{
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Ecommerce-PHP/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						...
						return ['status'=> 303, 'message'=> 'Falló'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Falló en subir la imagen'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Imagen pesada, Máximo 2 MB'];
			}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Ecommerce-PHP/product_images/".$uniqueImageName)) {
					...
					return ['status'=> 303, 'message'=> 'Falló la carga de imagen'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Imagen extensa, máximo 2MB'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Formato de imagen inválido [Formatos válidos : jpg, jpeg, png]'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Categoría ya existe'];
		}else{
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				...
				return ['status'=> 202, 'message'=> 'Falló'];
			}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'ID de categoría inválida'];
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			return ['status'=> 303, 'message'=>'ID de marca inválida'];
		}
