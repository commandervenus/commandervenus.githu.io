...
class Credentials
{
	...
	public function createAdminAccount($name, $email, $password){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Correo ya existe'];
		}else{
	...
	public function loginAdmin($email, $password){
		...
		if ($q->num_rows > 0) {
			...
			if (password_verify($password, $row['password'])) {
				...
				return ['status'=> 202, 'message'=> 'Ingreso Exitoso'];
			}else{
				return ['status'=> 303, 'message'=> 'Ingreso Fallido'];
			}
		}else{
			return ['status'=> 303, 'message'=> 'Cuenta no creada aún con este correo'];
		}
	}
